...
cmd:text()
cmd:text('Options:')
cmd:option('--dataDir', '/home/jchen16/NEW/data/fungus/encoder_FM16_X12', 'the directory to load')
...
cmd:option('--checkpoint',10000,'the number of iteration to save checkpoints')
cmd:option('--CheckPointDir','/home/jchen16/NEW/code/checkpoint','the directoty to save checkpoints')
cmd:option('--nIteration',400000,'the number of training iterations over all training data')
...
function train()
    ...
    for offset_idx=1, pindex:size(1) do
		...
		if opt.checkpoint>0 and epoch%opt.checkpoint==0 and epoch>10000 then
    		local filename=string.format('%s/v2_%d.bin',opt.CheckPointDir,epoch);
