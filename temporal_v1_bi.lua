...
cmd:option('--HiddenSize','{16,16}','size of hidden layers')
cmd:option('--XX',20,'XX')
cmd:text()
...
for i=1,#opt.HiddenSize do
	temporal_model.modules[i+1].backwardModule:initBias(1,0)
	temporal_model.modules[i+1].backwardModule:initBias(1,0)
end
...
function train()
    ...
    for offset_idx=1, pindex:size(1) do
		...
		if opt.checkpoint>0 and epoch%opt.checkpoint==0 and epoch>10000 then
    		local filename=string.format('%s/v1_bi_%d.bin',opt.CheckPointDir,epoch);
