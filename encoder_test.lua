...
cmd:text()
cmd:text('Options:')
cmd:option('--modelPath','/home/jchen16/NEW/code/checkpoint/unet_355000.000000.bin','path to the trained model')
cmd:option('--imageDir', '/home/jchen16/NEW/data/fungus/test', 'the directory to load')
cmd:option('--outputDir', '/home/jchen16/NEW/data/fungus/test_out', 'the directory to save the seg')
...
for i=1, #files do
    ...
    for xi=1,numX do
        ...
        for yi=1,numY do
            ...
            local tmp=torch.Tensor(1,1,windowSize,windowSize)
            tmp[1][1]=image_whole:sub(1,1,x0,x0+windowSize-1,y0,y0+windowSize-1)
            table.insert(tiles, tmp)
        end
    end
